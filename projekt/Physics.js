...
export class Physics {
        ...
        this.scene.traverse(node => {
            ...
            if (node.velocity) {
                if(node.camera){
                    ...
                    node.camera.parent.updateMatrix();
                }
                    ...
                    if (node !== other) {
                        if(node.collidable && other.collidable){
                            ...
                            if( (node.name === "Camera" && other.name === "Arrow") || 
                                ...
                                (node.name.includes("Cube") && other.name === "Ground") ||
                                (other.name.includes("Cube") && node.name === "Ground") ||
                                (node.name === "Camera" && other.name === "Ground") ||
                                (other.name === "Camera" && node.name === "Ground")){
                                //no collisions
                            }else{
                                //do collisions
            ...
            if(node instanceof Arrow){
                if(node.collidable){ //stops arrows from falling when they hit a target
                    ...
                    if(node.velocity[1] < 0.5){
                        quat.add(node.rotation, node.rotation, quat.fromValues(-0.001, 0, 0, 0));
                    }
                    node.updateMatrix();
                }
            }
    ...
    resolveCollision(a, b) {
        ...
        if(a.name.includes("pickup") && b.name === "Camera"){
            if(!a.pickedUp){
                a.visible = false;
                b.camera.ammo = b.camera.ammo + 0.5;
                a.pickedUp = true;
            }
        }else if(b.name.includes("pickup") && a.name === "Camera"){
            if(!a.pickedUp){
                b.visible = false;
        ...
        if((a.name === "Camera" && (b.name !== "Arrow" || b.name !== "arrow_container" || !(b.name.includes("pickup")) || b.name !== "Ground")) ||
           (b.name === "Camera" && (a.name !== "Arrow" || a.name !== "arrow_container" || !(a.name.includes("pickup")) || a.name !== "Ground"))){
            ...
            if(a.name === "Arrow" && b.name !== "Arrow" && !(b.name.includes("pickup"))){
                a.velocity = [0,0,0];
                a.collidable = false;
                if(b.name.includes("Target")){
                    b.hit = true;
                }
            }else if(b.name === "Arrow" && b.name !== "Arrow" && !(b.name.includes("pickup"))){
                b.velocity = [0,0,0];
                b.collidable = false;
